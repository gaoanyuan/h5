...
angular.module('app').factory('docService', ['$http', '$q', 'toaster', '$state', '$timeout', '$rootScope', 'ossService',
  function ($http, $q, toaster, $state, $timeout, $rootScope, ossService) {
    ...
    var s = {
      ...
      addElement: function(pageId, type, options) {
        var page = s.getPage(pageId);
        if (!page) return;
        var elem;
        switch(type) {
          case "submit":
            elem = jQuery.extend(true, {}, window.config.defaultButtonData, options);
            break;
          case "input":
            elem = jQuery.extend(true, {}, window.config.defaultInputData, options);
            break;
          case "text":
            elem = jQuery.extend(true, {}, window.config.defaultTextData, options);
            break;
          case "image":
            elem = jQuery.extend(true, {}, window.config.defaultImageData, options);
            break;
        }
        elem.id = uuid();
        // 获取到最大的zIndex
        ...
        angular.extend(elem.css, {
          zIndex: zIndex,
          left: (window.player.width - elem.css.width) / 2 + (angular.isString(elem.css.left)?Number(elem.css.left):0),
          top: (window.player.height - elem.css.height) / 2 + (angular.isString(elem.css.top)?Number(elem.css.top):0)
        });
