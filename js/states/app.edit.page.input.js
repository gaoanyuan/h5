...
angular.module('app').config(['$stateProvider',
  function ($stateProvider) {
    $stateProvider.state('app.edit.page.input', {
      url: '/input/:elementId',
      deepStateRedirect: {
        default: 'app.edit.page.input.property',
        params: true,
        fn: ['$dsr$', '$state', function($dsr$, $state) {
          ...
          if ($dsr$.redirect.state == 'app.edit.page.input.select') {
            return {
              state: 'app.edit.page.input.property',
              params: $dsr$.redirect.params
            };
          } else {
            return true;
    ...
    }).state('app.edit.page.input.select', {
      url: '/select',
      data: {
        tabs: [
          {
      ...
      views: {
        'tab': {
          ...
          controller: ['$state', '$stateParams', '$scope', 'docService', '$timeout', function ($state, $stateParams, $scope, docService, $timeout) {
            ...
            $scope.enableTransform();
            $scope.element.on('dblclick.input', function() {
              $state.go('^.select');
            });
            $scope.$on('$destroy', function () {
              $scope.disableTransform();
              $scope.element.off('dblclick.input');
            });
          }]
        }
      }
    }).state('app.edit.page.input.style', {
      url: '/style',
      data: {tag: 'input'},
      views: {
        'tab': {
    ...
    }).state('app.edit.page.input.arrange', {
      url: '/arrangement',
      data: {tag: 'text'},
      views: {
        'tab': {
    ...
    }).state('app.edit.page.input.animate', {
      url: '/animate',
      data: {tag: 'text'},
      views: {
        'tab': {
