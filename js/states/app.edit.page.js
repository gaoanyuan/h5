        ...
        "page-panel@app.edit": {
          ...
          controller: ['$state', '$stateParams', '$scope', '$rootScope', 'docService', 'multiselect', 'common',
            function ($state, $stateParams, $scope, $rootScope, docService, multiselect, common) {
              ...
              $scope.$on('ADD_INPUT', function (e) {
                var elem = docService.addElement($stateParams.pageId, 'input');
                setTimeout(function() {
                  $state.go('app.edit.page.input.property', {
                    elementId: elem.id
                  });
                }, 17);
              });
              $scope.$on('ADD_FORM', function (e) {
                docService.addElement($stateParams.pageId, 'input', {placeholder:"姓名", css:{top:"-100"}, label:"姓名", target:"userName"});
                docService.addElement($stateParams.pageId, 'input', {placeholder:"手机", css:{top:"-50"}, label:"手机", target:"mobileNo"});
                var elem = docService.addElement($stateParams.pageId, 'submit', {title:"提交"});
